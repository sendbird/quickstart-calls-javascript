...
export default class CallView extends BaseElement {
  ...
  build() {
    ...
    remoteUser.nickname = "1234567890123456789012345678901234567890";
    ...
    this.peerName = createLabel({ id: 'peer_name', innerText: (remoteUser.nickname ? remoteUser.nickname : remoteUser.userId), className: `${classes['peerName']} ${classes['fontBig']}` });
    ...
    const peerMuteLabel = createLabel({ id: 'peer_mute_label', className: `${classes['peerMuteLabel']} ${classes['fontSmall']}`, innerText: `${remoteUser.userId} audio muted this call` });
